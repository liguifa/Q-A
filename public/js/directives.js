		...
		return {
			...
			controller:function($http){
				this.getLabels = function(){
					return $http({
						method:"POST",
						url:"/index.php/Home/Index/Tages",
						// params:{
						// 	action:"getAllLables"
						// },
						headers:{
			...
			template : '<div class="label-expander mb-10">'+
							'<ul class="list-inline	question-tags">'+
								'<li>'+
									'<div class="">'+
										'<input type="text" class="question-tag-input" placeholder="标签，如：java" ng-keyup="inputKeyup()" ng-keydown="inputKeydown()"></input>'+
									'</div>'+
			...
			link : function(scope,iElement,iAttrs,controller){
				...
				var allTags = ["UWP","XAML","Win10ϵͳ"],
					promise = controller.getLabels(),
					newInputTemp = '<li>'+
										'<div class="">'+
											'<input type="text" class="question-tag-input" placeholder="标签，如：java" ng-focus></input>'+
										'</div>'+
										'<ul class="tags-layer"></ul>'+
									'</li>';
				promise.success(function(data){
